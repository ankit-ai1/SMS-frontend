...
/**
 * The header every principal screen opens with. The year chip sits opposite
 * the title exactly as it does on the admin attendance, fees and exam screens,
 * so the two sets of screens read as one product.
 */
...
/**
 * Says plainly that the screen only reads. Every principal screen carries it,
 * so the absence of edit controls reads as deliberate rather than missing.
 */
export function ViewOnlyChip({ className }: { className?: string }) {
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1.5 rounded-xl bg-muted px-2.5 py-1.5 text-xs font-medium text-muted-foreground ring-1 ring-border",
        className
      )}
    >
...
/** Sections, terms, exams and fees all hang off a year, so they share this. */
